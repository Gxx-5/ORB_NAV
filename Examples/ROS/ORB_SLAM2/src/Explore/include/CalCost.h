...
void init(ros::NodeHandle nodeHandler,double focal_len,double field_size,double resolution,ros::Publisher pub_cur_view_cloud,ros::Publisher vis_pub,ros::Publisher vis_text_pub,ros::Publisher costcloud_pub){
...
void mainProcess(ORB_SLAM2::System &SLAM,ros::Publisher pub_cur_view_cloud,geometry_msgs::Pose camera_pose){
	...
	costcube_map = COSTCUBE.calCostCubeByDistance(map_points_pos);
...
void VisualizeCostCube(cv::Mat cost_map){
	...
	marker.pose.orientation.z = 0.0;
	marker.pose.orientation.w = 1.0;
	marker.scale.x = 0.8 * _resolution;
	marker.scale.y = 0.8 * _resolution;
	marker.scale.z = 0.8 * _resolution;
	...
	marker_text.pose.orientation.z = 0.0;
	marker_text.pose.orientation.w = 1.0;
	marker_text.scale.z = 0.3 * _resolution;
	marker_text.color.a = 1.0; // Don't forget to set the alpha!!

	vector<int> cam_posid{int(_field_size / _resolution),int(_field_size / _resolution),0};
	...
	for (int row = 0; row < cost_map.size[0]; ++row){
		for (int col = 0; col < cost_map.size[1]; ++col){
				...
				// marker.pose.position.x = camera_pose.position.x + (row - cam_posid[0]) * _resolution;
				// marker.pose.position.y = camera_pose.position.y + (col - cam_posid[1]) * _resolution;
				// marker.pose.position.z = camera_pose.position.z + (hei - cam_posid[2]) * _resolution;
				marker.pose.position.x =  (row - cam_posid[0]) * _resolution;
				marker.pose.position.y =  (col - cam_posid[1]) * _resolution;
				marker.pose.position.z =  (hei - cam_posid[2]) * _resolution;
				marker.color.r =  color[0];
				marker.color.g = color[1];
				marker.color.b = color[2];
				marker.id = marker_id++;
	...
	int i = 0;
		...
		for (int col = 0; col < cost_map.size[1]; ++col)
                        for (int hei = 0;hei < cost_map.size[2]; ++ hei){
				cloud.points[i].x = (row - cam_posid[0]) * _resolution;
				cloud.points[i].y = (col - cam_posid[1]) * _resolution;
				cloud.points[i].z =  (hei - cam_posid[2]) * _resolution;
				float cur_cost = cost_map.at<float>(row, col, hei);
