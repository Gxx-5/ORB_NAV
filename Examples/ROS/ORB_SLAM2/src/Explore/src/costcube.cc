...
CostCube::CostCube(double radius,double res){
        filter_radius = radius;
        resoluion = res;
        voxel_n = 2 * filter_radius / resoluion;
...
void CostCube::Bresenham3D(const geometry_msgs::Point &pt_pos, cv::Mat &occupied,
        ...
        int z2 = int((pt_pos.z - cam_pos.z)/resoluion + voxel_n/2);
...
//Calculate average distance between current voxel and nearby obstacle. 
        ...
        if(occ_n <= 0){
                cout <<"No key points detected when calculate distance from voxel to obstacle! return -1" << endl;
                return -1;
        }
...
//     {
      // make sure cost falls off by Euclidean distance
        ...
        float cost = 255 * factor;//(unsigned char)((INSCRIBED_INFLATED_OBSTACLE - 1) * factor);
//     }
        return cost;
  }